...
class CaptioningRNN(object):
    ...
    def sample(self, features, max_length=30):
        ...
        h0 = np.dot(features, W_proj) + b_proj
        V, D = W_embed.shape
        word_idx = np.repeat(self._start, N) # (N, )
        if self.cell_type == 'lstm':
            c0 = np.zeros_like(h0)
        for t in range(max_length):
            x = W_embed[word_idx] # (N, wordvec_dim)
            if self.cell_type == 'rnn':
                h0, _ = rnn_step_forward(x, h0, Wx, Wh, b) # (N, H)
            elif self.cell_type == 'lstm':
                h0, c0, _ = lstm_step_forward(x, h0, c0, Wx, Wh, b) # (N, H)
            scores = np.dot(h0, W_vocab) + b_vocab # (N, V)

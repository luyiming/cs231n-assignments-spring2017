...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        hidden_layer, aff_relu_cache = affine_relu_forward(X, W1, b1)
        scores, aff_cache = affine_forward(hidden_layer, W2, b2)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dX1, dW2, db2 = affine_backward(dscores, aff_cache)
        dX, dW1, db1 = affine_relu_backward(dX1, aff_relu_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
            self.params['W' + str(i + 1)] = np.random.randn(all_dims[i], all_dims[i + 1]) * weight_scale
            self.params['b' + str(i + 1)] = np.zeros(all_dims[i + 1])
        
        if use_batchnorm:
            for i in range(self.num_layers - 1):
                self.params['gamma' + str(i + 1)] = np.ones(self.params['b' + str(i + 1)].shape)
                self.params['beta' + str(i + 1)] = np.zeros(self.params['b' + str(i + 1)].shape)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
